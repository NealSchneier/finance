...
from models import Sector, Industry, Quotes, Company, Companies
...
def sectors(request, atr=None, year=None, month=None, day=None, toYear=None, toMonth=None, toDay=None):  
  if toYear != None and toMonth !=None and toDay !=None:
    query = 'select * from sector where date(curr_date) >= "%s-%s-%s" and date(curr_date) <= "%s-%s-%s" order by %s' % (str(year), str(month), str(day), str(toYear), str(toMonth), str(toDay), atr)
    sectors = Sector.objects.raw(query)
  elif atr != None and year != None and month != None and day != None:
    query = 'select * from sector where date(curr_date) = "%s-%s-%s" order by %s' % (str(year), str(month), str(day), atr)
    sectors = Sector.objects.raw(query)
  else:
...
def industry(request, atr=None, year=None, month=None, day=None, toYear=None, toMonth=None, toDay=None):  
  if toYear != None and toMonth !=None and toDay !=None:
    query = 'select * from industry where date(curr_date) >= "%s-%s-%s" and date(curr_date) <= "%s-%s-%s" order by %s' % (str(year), str(month), str(day), str(toYear), str(toMonth), str(toDay), atr)
    data = Industry.objects.raw(query)
  elif atr != None and year != None and month != None and day != None:
    query = 'select * from industry where date(curr_date) = "%s-%s-%s" order by %s' % (str(year), str(month), str(day), atr)  
    data = Industry.objects.raw(query)
  else:
